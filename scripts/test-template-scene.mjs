...
const groups = {
  'recipe-server': ['test/sceneSimplePosePromptPolicy.test.js'],
  'recipe-client': ['src/features/scene-builder/scenePoseRecipeModel.test.ts'],
...
if (part !== 'all' && !Object.hasOwn(groups, part)) throw new Error(`Use --part=${Object.keys(groups).join('|')}|all`);
for (const name of part === 'all' ? Object.keys(groups) : [part]) {
  const server = name === 'server' || name === 'recipe-server';
