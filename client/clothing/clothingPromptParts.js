/**
 * ModelPromptForge - Client-side Clothing Prompt Helper
 */
(function () {
  ...
  function getClothingSourceOwnership(selections, referenceState, mode) {
  ...
  function compileClothingPromptParts(selections, referenceState, mode, referenceOverrides = null) {
    ...
    if (ownership === 'upload') {
      ...
      if (overrides.enabled) {
        if (overrides.primaryColor) {
          const value = selections?.['Primary Color']?.value;
          if (value) parts.push(`changing the primary garment color to ${value}`);
        }
        if (overrides.secondaryColor) {
          const value = selections?.['Secondary Color']?.value;
          if (value) parts.push(`changing the secondary garment color to ${value}`);
        }
        if (overrides.pattern) {
          const value = selections?.Pattern?.value;
          if (value) parts.push(`applying ${value}`);
        }
        if (overrides.material) {
          const value = (selections?.Material || selections?.['Material / Surface'])?.value;
          if (value) parts.push(`rendering the garment with ${value}`);
        }
  ...
  function cleanupClothingPromptParts(parts) {
  ...
  function isCanonicalOutfitBase(outfitBase) {
  ...
  function normalizeReferenceOverrides(value) {
  ...
  window.ModelPromptForgeClothingPromptParts = {
    getClothingSourceOwnership,
    compileClothingPromptParts,
    cleanupClothingPromptParts,
    isCanonicalOutfitBase,
    normalizeReferenceOverrides
  };
})();
