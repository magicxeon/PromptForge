...
(() => {
  ...
  function render() {
    ...
    page.innerHTML = `<section class="community-home-hero"><div class="community-home-copy"><span class="community-home-kicker">${translate('community.home.kicker', 'COMMUNITY')}</span><h2>${translate('community.home.title', 'Discover ideas, then make them yours.')}</h2><p>${translate('community.home.description', 'Explore reusable image workflows, save inspiration and move directly into the right creation surface.')}</p><div class="community-home-actions"><button type="button" class="btn-neon-outline" data-route="/community/characters">${translate('character-profiles.community.browse', 'Browse Characters')}</button><button type="button" class="btn-neon-outline" data-route="/history">${translate('community.home.library', 'Open my library')}</button><button type="button" class="btn-neon-outline" data-community-own-profile hidden>${translate('community.creator.myProfile', 'My creator profile')}</button></div></div><aside class="community-create-panel"><div><span class="community-home-kicker">${translate('community.create.kicker', 'CREATE')}</span><h3>${translate('community.create.title', 'Start a new image')}</h3><p>${translate('community.create.description', 'Choose the amount of guidance you want.')}</p></div><div class="community-create-actions"><button type="button" data-community-workflow="headshot">${translate('community.create.headshot', 'Headshot Grid')}</button><button type="button" data-community-workflow="character">${translate('community.create.character', 'Build Character')}</button><button type="button" data-community-workflow="scene">${translate('community.create.scene', 'Scene Builder')}</button><button type="button" data-community-workflow="playground">${translate('community.create.playground', 'Open Playground')}</button></div></aside></section><section class="community-feed-section" data-community-feed></section>`;
