...
export function CreditsRoute() {
  ...
  const account = useQuery({ queryKey: ['credit-account', actorId], queryFn: getCreditAccount, enabled: Boolean(actor) });
  ...
  const grant = useMutation({
    mutationFn: () => grantMockCredits(10),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['credit-account', actorId] });
  ...
  return (
    <main>
      <header className="mb-5 border-b border-[var(--mpf-border)] pb-5"><span className="text-xs font-bold uppercase text-cyan-300">{t('ui.credits.kicker')}</span><h1 className="mb-2 mt-2 text-3xl">{t('ui.credits.title')}</h1><p className="m-0 text-sm text-[var(--mpf-text-muted)]">{t('ui.credits.description')}</p></header>
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Surface className="p-5"><strong className="block text-4xl">{account.data.account.reservedCredits}</strong><span className="text-sm text-[var(--mpf-text-muted)]">{t('ui.credits.reserved')}</span>{import.meta.env.DEV ? <Button className="mt-5" icon={<Plus className="size-4" />} disabled={grant.isPending} onClick={() => grant.mutate()}>{t('ui.credits.addMock')}</Button> : null}</Surface>
      </div>
