...
export class CinematicDataLineageService {
  build(project, { sceneId = null, shotId = null } = {}) {
    ...
    const videoPackets = attempts.filter(isVideoAttempt).map(attempt => ({
      ...
      commercialOperation: attempt.commercialOperation || attempt.operation || null,
      inputMode: attempt.inputMode || (attempt.sourceFingerprint ? 'image_to_video' : null),
      keyframeContractFingerprint: attempt.keyframeContractFingerprint || null,
      videoPacketFingerprint: attempt.videoPacketFingerprint || null,
      promptStrategyId: attempt.promptStrategyId || null,
      promptStrategyVersion: attempt.promptStrategyVersion || null,
      renderedPromptFingerprint: attempt.renderedPromptFingerprint || null,
      referencePlanFingerprint: attempt.referencePlanFingerprint || null,
      technicalProbeStatus: attempt.outputAsset?.technicalProbe?.status || null,
      sourceFingerprint: attempt.sourceFingerprint || null,
      status: attempt.status, downstreamSourceStatus: attempt.downstreamSourceStatus || 'current'
    }));
