...
test('Cinematic data lineage deterministically traces a complete Setup-to-Finish Project', () => {
  ...
  assert.equal(first.videoPackets[0].quoteId, 'vest_video_single');
  assert.equal(first.videoPackets[0].qualificationAuthorizationId, 'vqual_video_single');
  assert.equal(first.videoPackets[0].settlementStatus, 'qualification_no_charge');
  assert.equal(first.videoPackets[0].technicalProbeStatus, 'passed');
