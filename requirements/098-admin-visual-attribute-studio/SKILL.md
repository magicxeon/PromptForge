...
3. Every applicable file in `requirements/098-admin-visual-attribute-studio/`, in
   sequence.
4. `requirements/003-implementation-visual-character-builder-plan/` for the
   current semantic and visual contracts.
5. Nearest Attribute loader, compiler, React model, visual registry and tests.
...
2. Identify the stable category, field and option IDs.
3. Extend the canonical schema before UI or persistence.
4. Add server validation and repository behavior.
5. Add API response schemas and actor-scoped Query keys.
6. Build UI with shared controls and exact customer-card preview.
7. Route AI asset work through Generation and Assets.
8. Add compatibility and prompt fixtures for all affected modes.
9. Verify imported catalog parity before changing runtime authority.
10. Publish through an immutable release and test rollback.
...
- Do not publish an option with missing assets unless an explicit text fallback
  has passed review.
