# 20. AI Model Comparison Workspace

**ID:** `020-ai-model-comparison-workspace`  
**Application:** `ModelPromptForge`  
**Status:** Implemented - pending runtime and credential verification  
...
**Created:** 2026-07-14  
**Implementation authorization:** Approved by user and implemented on 2026-07-14
...
- [x] สำรวจ ProviderFactory, Queue Manager, History, Collections และ credit lifecycle ปัจจุบัน
- [x] ออกแบบ Provider Registry และ Model Capability schema
- [x] ออกแบบ Comparison Set/Run repository interface
- [x] กำหนด estimate/confirm/idempotency contract
- [x] เพิ่ม unit tests สำหรับ validation และ aggregate status

### Phase B: Server orchestration

- [x] เพิ่ม Comparison repository แบบ serialized persistence
- [x] เพิ่ม Comparison Manager/Orchestrator
- [x] เพิ่ม estimate endpoint
- [x] เพิ่ม create/list/detail/update endpoints
- [x] เชื่อม 2-4 child jobs เข้ากับ Queue Manager
- [x] เพิ่ม persistent status recovery และ partial failure handling
- [x] เพิ่ม aggregate polling contract
- [x] เพิ่ม serialized credit ledger และ deduction/refund tests

### Phase C: Model selector UX

- [x] เพิ่ม Compare icon ใกล้ AI Model selector
- [x] เพิ่ม localized behavior และ accessibility attributes
- [x] เพิ่ม active state และ slot count badge
- [x] เพิ่ม Comparison Model Tray เริ่มต้น 2 slots
- [x] เพิ่ม/ลบ/reorder slots สูงสุด 4
- [x] แสดง capabilities, warnings และ cost estimate ต่อ slot
- [x] เพิ่ม confirmation ก่อน Generate Comparison
- [x] persist Comparison draft แยกจาก Single Image state

### Phase D: Comparison Workspace

- [x] เพิ่ม responsive 2/3/4-image layout และ mobile horizontal workspace
- [x] แสดง independent job status และ partial results
- [x] เพิ่ม normalized synchronized zoom/pan
- [x] เพิ่ม Fit, 100%, Reset, fullscreen และ sync toggle
- [x] เพิ่ม slot actions และ Winner selection
- [x] รองรับ reload/reconnect ระหว่างงานด้วย persistent Run และ polling

### Phase E: History and Collections

- [x] เพิ่ม Comparison badge และ Set link ใน History
- [x] เพิ่ม individual/add-all Collection actions
- [x] เพิ่ม Comparison Set/Run/Slot grouping metadata ใน child History
- [x] เพิ่ม cleanup/repair rules สำหรับ History, result และ Winner
- [x] เพิ่ม reopen Comparison Set จาก History และ Comparison Set drawer

### Phase F: Verification

- [x] Unit tests: slot limits, duplicate models, registry validation
- [x] Unit tests: aggregate status and partial failures
- [x] Unit tests: estimate, deduction and refund totals
- [x] Persistence tests: concurrent child updates do not lose data
...
- เพิ่ม `server/comparison/` แยก Repository, Validator และ Orchestrator ออกจาก route bootstrap
- เพิ่ม `server/creditManager.js` เพื่อ serialize credit mutation และบันทึก ledger ต่อ child job
- แยก normalization/prompt compile/queue options ไว้ที่ `server/generationRequestService.js` ให้ Single และ Comparison ใช้ contract เดียวกัน
- ใช้ polling เป็น aggregate progress contract ใน MVP แทน SSE โดย child jobs ยังใช้ Queue Manager เดิม
- เพิ่ม `client/comparison.js` เป็น module controller แยกจาก `client/app.js`; ไฟล์หลัก expose bridge เฉพาะ capability ที่จำเป็น
- `Comparison Set` หนึ่งรายการรองรับหลาย Runs ใน data model แต่ UI รุ่นแรกสร้าง Set ใหม่ต่อการยืนยันหนึ่งครั้ง
- Collection grouping อ่านจาก Comparison metadata ของ child History โดยยังไม่เปลี่ยน Collection storage schema
- เพิ่ม shared `AppDialog` ตาม theme เดิมและยกเลิก native `alert`, `confirm` และ `prompt` ทั้งหมดทั้ง main client และ `sub-app-game-character` รวมถึง cost confirmation, rename และ Collection picker ของ Comparison Workspace
- เพิ่ม `server/comparisons.json` ใน `nodemon.json` ignore list เพื่อไม่ให้ persistence write restart process และทำลาย in-memory child queue
- เพิ่ม idempotent lost-job refund สำหรับ Comparison jobs ที่ถูก server restart ตัดกลางทาง
- เมื่อปิด Workspace งานยัง polling ต่อ พร้อม active Comparison chip ใกล้ Model selector และ parent batch card ใน Background Queue สำหรับกลับเข้าดู Run เดิม
