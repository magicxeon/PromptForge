...
The current Casting layout is `character-casting-three-view-v4`: **front view,
exact side profile and back view**, arranged side by side. Existing v2 assets
remain valid historical references.
...
- Clothing selections and Outfit references are absent from UI state and
  request payload.
- The server locks the versioned medium-gray white-grid silhouette-reading outfit defined
  by Character Profile Requirement 010. Female, male and neutral presentations
  retain professional opaque coverage while making selected proportions clear.
- New casting output uses a seamless light warm-gray studio background and no
  contour grid, measurements, text, or isolated foot inset.
...
- Body controls may describe explicit adult fashion-model proportions,
  ...
  silhouette never changes the locked modest black casting outfit into
...
Output: three-view photorealistic full-body black-outfit casting reference
```
...
- Reusable Character prompt contains three views, the locked black
  silhouette-reading outfit and light warm-gray background while excluding
  selected Clothing.
...
2. Generate Reusable Model and verify three complete views with the locked
   medium-gray white-grid silhouette-reading casting outfit on light warm-gray.
